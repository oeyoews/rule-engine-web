...
const rule = computed(() => {
  ...
  if (props.data?.ruleName) {
    const foundRule = editorStore.rules.find(r => r.name === props.data?.ruleName)
    if (foundRule) {
      return foundRule
    }
  }
  ...
  if (index !== undefined && index >= 0 && index < editorStore.rules.length) {
    return editorStore.rules[index]
  }
  return null
})
