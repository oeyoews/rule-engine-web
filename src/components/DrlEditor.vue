...
import {
  ...
  CheckCircle, XCircle, Wand2, Eye, Terminal, Upload, Grip, Power, RotateCcw, Lock
    ...
    if (!advancedMode.value) {
      ...
      if (hasModification) {
        ...
        ElMessageBox.confirm(
          '检测到您在高级模式中有未保存的修改，切换到高级模式将覆盖这些修改。是否继续？',
          '确认覆盖',
          {
            confirmButtonText: '覆盖修改',
            cancelButtonText: '取消',
            type: 'warning',
          ...
          // 用户取消，切换回原标签页
          activeTab.value = oldTab || 'config'
        })
      } else {
        // 高级模式中没有修改，直接同步最新的表单数据
        manualDrlCode.value = autoGeneratedCode.value
      }
    }
  } else if (oldTab === 'advanced' && newTab !== 'advanced') {
    // 从高级模式标签页切换到其他标签页时，检查是否需要同步代码
    const currentCode = manualDrlCode.value.trim()
    const generatedCode = autoGeneratedCode.value.trim()
    const hasModification = currentCode !== generatedCode
                ...
                <div>
                  ...
                  <div class="mt-3 flex items-start gap-3 text-sm text-red-700 bg-linear-to-r from-red-50 to-orange-50 border-l-4 border-red-500 rounded-lg p-3 shadow-sm">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-gray-700 leading-relaxed">您正在直接编辑 DRL 代码。修改将影响下载和复制的结果。切换到其他标签页可返回表单编辑。</p>
                    </div>
                  </div>
                </div>
